...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil5_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil5.nsls2.bnl.local", 10)
...
# Table Horiz Upstream
GalilCreateAxis("$(PORT)", "E", 1, "", 1)
# Table Horiz Downstream
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil5.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil5.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-ES{Galil:5}")
...
save_restoreSet_status_prefix("XF:19IDC-ES{Galil:5}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-ES{Galil:5}")
...
dbpf XF:19IDC-ES{Galil:5}ECATFLT_STATUS.SCAN "Passive"
