...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil0_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil13.nsls2.bnl.local", 10)
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil0.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil0.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-TST{Galil:0}")

# Vector Scanning
dbLoadRecords("$(TOP)/db/shutter.db",     "P=XF:19IDC-TST,R={Gon:1-Sht},PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vector.db",      "P=XF:19IDC-TST,R={Gon:1-Vec},PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vectorMotor.db", "P=XF:19IDC-TST,R={Gon:1-Vec},M=O,N=0,EGU=deg,PREC=3,MTR=XF:19IDC-TST{Gon:1-Ax:O}Mtr,PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vectorMotor.db", "P=XF:19IDC-TST,R={Gon:1-Vec},M=X,N=1,EGU=mm,PREC=3,MTR=XF:19IDC-TST{Gon:1-Ax:X}Mtr,PORT=$(PORT)")
...
save_restoreSet_status_prefix("XF:19IDC-TST{Galil:0}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-TST{Galil:0}")
...
dbpf XF:19IDC-TST{Galil:0}ECATFLT_STATUS.SCAN "Passive"
