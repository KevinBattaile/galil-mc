...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil11_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil11.nsls2.bnl.local", 10)
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil11.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil11.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-OP{Galil:11}")
...
save_restoreSet_status_prefix("XF:19IDC-OP{Galil:11}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-OP{Galil:11}")
...
dbpf XF:19IDC-OP{Galil:11}ECATFLT_STATUS.SCAN "Passive"
