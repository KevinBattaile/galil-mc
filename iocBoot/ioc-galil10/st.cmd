...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil10_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil10.nsls2.bnl.local", 10)
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil10.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil10.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-OP{Galil:10}")
...
save_restoreSet_status_prefix("XF:19IDC-OP{Galil:10}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-OP{Galil:10}")
...
dbpf XF:19IDC-OP{Galil:10}ECATFLT_STATUS.SCAN "Passive"
