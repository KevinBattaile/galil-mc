...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil1_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil1.nsls2.bnl.local", 10)
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil1.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil1.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-ES{Galil:1}")
...
save_restoreSet_status_prefix("XF:19IDC-ES{Galil:1}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-ES{Galil:1}")
...
dbpf XF:19IDC-ES{Galil:1}ECATFLT_STATUS.SCAN "Passive"
