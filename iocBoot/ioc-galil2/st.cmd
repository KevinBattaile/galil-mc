...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil2_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil2.nsls2.bnl.local", 10)
...
# Sample X
GalilCreateAxis("$(PORT)", "E", 0, "", 1)
# Sample Y
GalilCreateAxis("$(PORT)", "F", 0, "", 1)
# Sample Z
GalilCreateAxis("$(PORT)", "G", 0, "", 1)
# Kappa
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil2.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil2.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-ES{Galil:2}")

# Vector Scanning
dbLoadRecords("$(TOP)/db/shutter.db",     "P=XF:19IDC-ES,R={Gon:1-Sht},PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vector.db",      "P=XF:19IDC-ES,R={Gon:1-Vec},PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vectorMotor.db", "P=XF:19IDC-ES,R={Gon:1-Vec},M=O,N=0,EGU=deg,PREC=3,MTR=XF:19IDC-ES{Gon:1-Ax:O}Mtr,PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vectorMotor.db", "P=XF:19IDC-ES,R={Gon:1-Vec},M=X,N=1,EGU=mm,PREC=3,MTR=XF:19IDC-ES{Gon:1-Ax:X}Mtr,PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vectorMotor.db", "P=XF:19IDC-ES,R={Gon:1-Vec},M=Y,N=2,EGU=mm,PREC=3,MTR=XF:19IDC-ES{Gon:1-Ax:Y}Mtr,PORT=$(PORT)")
dbLoadRecords("$(TOP)/db/vectorMotor.db", "P=XF:19IDC-ES,R={Gon:1-Vec},M=Z,N=3,EGU=mm,PREC=3,MTR=XF:19IDC-ES{Gon:1-Ax:Z}Mtr,PORT=$(PORT)")
...
save_restoreSet_status_prefix("XF:19IDC-ES{Galil:2}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-ES{Galil:2}")
...
dbpf XF:19IDC-ES{Galil:2}ECATFLT_STATUS.SCAN "Passive"
