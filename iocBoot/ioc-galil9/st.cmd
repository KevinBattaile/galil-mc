...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil9_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil9.nsls2.bnl.local", 10)
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil9.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil9.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-OP{Galil:9}")
...
save_restoreSet_status_prefix("XF:19IDC-OP{Galil:9}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-OP{Galil:9}")
...
dbpf XF:19IDC-OP{Galil:9}ECATFLT_STATUS.SCAN "Passive"
