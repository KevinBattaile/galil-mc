...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil7_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil7.nsls2.bnl.local", 10)
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil7.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil7.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-OP{Galil:7}")
...
save_restoreSet_status_prefix("XF:19IDC-OP{Galil:7}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-OP{Galil:7}")
...
dbpf XF:19IDC-OP{Galil:7}ECATFLT_STATUS.SCAN "Passive"
