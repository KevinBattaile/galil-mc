...
#epicsEnvSet("GALIL_DEBUG_FILE", "/tmp/galil8_debug.txt")
...
GalilCreateController("$(PORT)", "xf19id2-galil8.nsls2.bnl.local", 10)
...
GalilStartController("$(PORT)", "$(TOP)/galil-config/19id-galil8.dmc", 0, 0)

dbLoadRecords("$(TOP)/db/galil8.db","PORT=$(PORT)")
dbLoadRecords("$(IOCSTATS)/db/iocAdminSoft.db", "IOC=XF:19IDC-OP{Galil:8}")
...
save_restoreSet_status_prefix("XF:19IDC-OP{Galil:8}")
...
dbLoadRecords("$(AUTOSAVE)/db/save_restoreStatus.db","P=XF:19IDC-OP{Galil:8}")
...
dbpf XF:19IDC-OP{Galil:8}ECATFLT_STATUS.SCAN "Passive"
